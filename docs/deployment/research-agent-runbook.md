...
- Anonymous RAG sessions use `RAG_SESSION_RETENTION_DAYS=30`. Authenticated long-term memory is created only by an explicit save and uses `MEMORY_RETENTION_DAYS=180`. The scheduled `agent_runtime.tasks.cleanup_expired_memory_task` runs daily at 03:00 in `TIME_ZONE` after the scheduler has registered its Beat entry.
